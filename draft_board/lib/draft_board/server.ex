defmodule DraftBoard.Server do
  ...
  get "/cfb" do
    ...
    query = if search && search != "", do: from(p in query, where: ilike(p.name, ^"%#{search}%")), else: query
  ...
  defp nav do
    """
    <div class="nav">
      <a href="/">CFB Players</a>
      <a href="/nfl">NFL Players</a>
      <a href="/teams">CFB Teams</a>
    </div>
    """
  end
  ...
  defp styles do
    ...
    <style>
      ...
      input[type="text"] { background: #1a1a2e; color: #e0e0e0; border: 1px solid #7B68EE; padding: 8px 12px; border-radius: 4px; width: 200px; }
      button { background: #7B68EE; color: white; border: none; padding: 8px 20px; border-radius: 4px; cursor: pointer; }
    </style>
    """
  end

  defp render_cfb(players) do
    ...
    <!DOCTYPE html>
    <html>
    <head><title>Draft Board - CFB</title>#{styles()}</head>
    <body>
      <h1>Draft Board — College Football</h1>
      #{nav()}
      <form class="filters" action="/cfb" method="get">
      ...
      <table>
        <thead>
          <tr>
            <th>Name</th><th>Position</th><th>School</th><th>Conference</th>
            <th>Grade</th><th>Score</th><th>Yds/G</th><th>TD/G</th><th>Rec/G</th>
          </tr>
        </thead>
        <tbody>#{rows}</tbody>
      </table>
  ...
  defp render_nfl(players) do
    ...
    <!DOCTYPE html>
    <html>
    <head><title>Draft Board - NFL</title>#{styles()}</head>
    <body>
      <h1>Draft Board — NFL (2026 Season)</h1>
      #{nav()}
      <form class="filters" action="/nfl" method="get">
        <input type="text" name="search" placeholder="Search player name..." />
        <select name="position">
          <option value="">All Positions</option>
          <option value="QB">QB</option>
      ...
      <table>
        <thead>
          <tr>
            <th>Name</th><th>Position</th><th>Team</th>
            <th>Grade</th><th>Score</th><th>Yds/G</th><th>TD/G</th><th>Rec/G</th><th>Catches/G</th>
          </tr>
        </thead>
        <tbody>#{rows}</tbody>
      </table>
  ...
  defp render_teams(teams) do
    ...
    <!DOCTYPE html>
    <html>
    <head><title>Draft Board - Teams</title>#{styles()}</head>
    <body>
      <h1>Draft Board — CFB Team Offense Rankings</h1>
      #{nav()}
      <table>
        <thead>
          <tr>
            <th>Rank</th><th>School</th><th>Conference</th>
            <th>Score</th><th>TDs/G</th><th>Yds/G</th><th>TO/G</th>
          </tr>
        </thead>
        <tbody>#{rows}</tbody>
      </table>
